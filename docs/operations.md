...
## Admin web console (ใช้ง่ายสุด)
เปิด `/admin` บนโดเมนเดียวกับ API → ใส่ `ADMIN_TOKEN` (เก็บในเบราว์เซอร์เครื่องนั้นเท่านั้น) แล้ว:
- ดูรายการกิจกรรม + สถานะความพร้อม/จำนวนรูป
- สร้าง/แก้ไขกิจกรรม (รหัส ชื่อ วันที่ โฟลเดอร์ต้นฉบับ)
- หน้ารายละเอียด `/admin/e/<id>`: เปิด/ปิด, ตั้งสาธารณะ/ไม่สาธารณะ, **สั่งเตรียมรูป + ดูความคืบหน้าเรียลไทม์**,
  ลิงก์ + **QR (สร้างในเครื่อง ไม่ส่ง URL ออกภายนอก)**, ถอน/คืนรูปด้วย photoId
> การสั่งเตรียมรูปจากคอนโซลรันเป็น background ใน API process (สะดวกสำหรับงานเล็ก/ทดสอบ)
> โปรดักชันปริมาณมากแนะนำ Cloud Run **Job** (worker แยก) เพื่อไม่แย่งทรัพยากรตอนค้นหา

## สร้างกิจกรรม (ทางเลือกอื่น)
- ผ่าน Apps Script `upsertEvent` หรือ API โดยตรง:
```bash
